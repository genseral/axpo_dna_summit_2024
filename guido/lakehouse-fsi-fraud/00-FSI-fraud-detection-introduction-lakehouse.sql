...
-- MAGIC
-- MAGIC The Lakehouse allow you to centralize all your data, from customer & retail banking data to real time fraud detection, providing operational speed and efficiency at a scale never before possible.
-- MAGIC
-- MAGIC
-- MAGIC ### Simple
...
-- MAGIC <img width="1px" src="https://ppxrzfxige.execute-api.us-west-2.amazonaws.com/v1/analytics?category=lakehouse&org_id=3391241334329287&notebook=%2F00-FSI-fraud-detection-introduction-lakehouse&demo_name=lakehouse-fsi-fraud&event=VIEW&path=%2F_dbdemos%2Flakehouse%2Flakehouse-fsi-fraud%2F00-FSI-fraud-detection-introduction-lakehouse&version=1">
...
-- MAGIC
-- MAGIC In this demo, we'll build a end 2 end Banking platform, collecting data from multiple sources in real time. 
-- MAGIC
-- MAGIC With this information, not only we'll be able to analyse existing past fraud and understand common patterns, but we'll also be able to rate financial transaction risk in realtime.
-- MAGIC
...
-- MAGIC 4. Build ML model & deploy them to provide real-time fraud detection capabilities.
-- MAGIC
-- MAGIC ### Our dataset
-- MAGIC
...
-- MAGIC
-- MAGIC - Banking ransaction history
...
-- MAGIC %md 
-- MAGIC Open the FSI banking & Fraud  <a dbdemos-pipeline-id="dlt-fsi-fraud" href="#joblist/pipelines/4fdb9319-96ec-4c50-8502-c1bc9808823e" target="_blank">Delta Live Table pipeline</a> or the [SQL notebook]($./01-Data-ingestion/01.1-DLT-fraud-detection-SQL) *(Alternatives: DLT Python version Soon available)*. <br>
...
-- MAGIC <br/><br/><br/>
-- MAGIC <div style="padding-left: 420px">
-- MAGIC   Now that our first tables have been created, we need to grant our Data Analyst team READ access to be able to start analyzing our banking databse information.
-- MAGIC   
...
-- MAGIC %md 
-- MAGIC Open the [Datawarehousing notebook]($./03-BI-data-warehousing/03-BI-Datawarehousing-fraud) to start running your BI queries or access or directly open the <a dbdemos-dashboard-id="fraud-detection" href='/sql/dashboardsv3/01efa8b9a3981709bf9e6d87fdd0dbb8' target="_blank">Banking fraud analysis dashboard</a>

-- COMMAND ----------

-- MAGIC %md-sandbox
-- MAGIC ## 4/ Predict Fraud risk with Data Science & Auto-ML
-- MAGIC
...
-- MAGIC
-- MAGIC However, knowing what we had Fraud in the past isn't enough. We now need to take it to the next level and build a predictive model to detect potential threat before they happen, reducing our risk and increasing customer satisfaction.
-- MAGIC
...
-- MAGIC Let's see how Databricks accelerate ML project with AutoML : 1 click model training with the [04.1-AutoML-FSI-fraud]($./04-Data-Science-ML/04.1-AutoML-FSI-fraud)
-- MAGIC
-- MAGIC ### ML: Realtime Model Serving
-- MAGIC
-- MAGIC Once our model trained and available, Databricks Model Serving can be used to enable real time inferences, allowing Fraud detection in real time.
-- MAGIC
...
-- MAGIC
-- MAGIC As result, our analyst team was able to simply build a system to not only understand but also forecast future failures and take action accordingly.
-- MAGIC
