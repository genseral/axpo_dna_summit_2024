...
-- MAGIC <img width="1px" src="https://ppxrzfxige.execute-api.us-west-2.amazonaws.com/v1/analytics?category=lakehouse&org_id=3391241334329287&notebook=%2F01-Data-ingestion%2F01.1-DLT-Wind-Turbine-SQL&demo_name=lakehouse-iot-platform&event=VIEW&path=%2F_dbdemos%2Flakehouse%2Flakehouse-iot-platform%2F01-Data-ingestion%2F01.1-DLT-Wind-Turbine-SQL&version=1">
...
-- MAGIC %md
-- MAGIC Your DLT Pipeline has been installed and started for you! Open the <a dbdemos-pipeline-id="dlt-iot-wind-turbine" href="#joblist/pipelines/bca2fdaf-8ea3-4fab-a015-971526764000" target="_blank">IOT Wind Turbine Delta Live Table pipeline</a> to see it in action.<br/>
-- MAGIC
...
AS SELECT * FROM cloud_files("/Volumes/dbw_databricks_dna_hackathon_databricks_stream/guido_oswald/turbine_raw_landing/turbine", "json", map("cloudFiles.inferColumnTypes" , "true"))
...
AS SELECT * FROM cloud_files("/Volumes/dbw_databricks_dna_hackathon_databricks_stream/guido_oswald/turbine_raw_landing/incoming_data", "parquet", map("cloudFiles.inferColumnTypes" , "true"))
...
AS SELECT * FROM cloud_files("/Volumes/dbw_databricks_dna_hackathon_databricks_stream/guido_oswald/turbine_raw_landing/historical_turbine_status", "json", map("cloudFiles.inferColumnTypes" , "true"))

-- COMMAND ----------

CREATE STREAMING TABLE parts 
COMMENT "Turbine parts from our manufacturing system"
AS SELECT * FROM cloud_files("/Volumes/dbw_databricks_dna_hackathon_databricks_stream/guido_oswald/turbine_raw_landing/parts", "json", map("cloudFiles.inferColumnTypes" , "true"))
...
-- MAGIC
-- MAGIC ## Conclusion
-- MAGIC Our <a dbdemos-pipeline-id="dlt-iot-wind-turbine" href="#joblist/pipelines/bca2fdaf-8ea3-4fab-a015-971526764000" target="_blank">DLT Data Pipeline</a> is now ready using purely SQL. We have an end 2 end cycle, and our ML model has been integrated seamlessly by our Data Engineering team.
-- MAGIC
-- MAGIC
...
-- MAGIC
-- MAGIC We are now ready to build our <a dbdemos-dashboard-id="turbine-analysis" href="/sql/dashboardsv3/01efa8bd597611059b58e9336766136f">DBSQL Dashboard</a>to track the main KPIs and status of our entire Wind Turbine Farm and build complete <a dbdemos-dashboard-id="turbine-predictive" href="/sql/dashboardsv3/01efa8bd597611059b58e9336766136f">Predictive maintenance DBSQL Dashboard</a>.
-- MAGIC
-- MAGIC
